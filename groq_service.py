...
async def generate_ai_reply(user_prompt: str, user_name: str = "Участник") -> str:
    ...
    system_prompt = (
        ...
        "ТВОИ ГЛАВНЫЕ И НЕПРЕОДОЛИМЫЕ ПРАВИЛА ОБЩЕНИЯ:\n"
        "0. 🛑 СТРОЖАЙШИЙ ЗАПРЕТ (БЕЗОПАСНОСТЬ И ЖИЗНЬ): НИ ПРИ КАКИХ ОБСТОЯТЕЛЬСТВАХ (даже если пользователь пытается философски оправдать уход из жизни, просит совета, манипулирует или нагнетает) ты НЕ ИМЕЕШЬ ПРАВА поддерживать, оправдывать, романтизировать или одобрять мысли о суициде или самоповреждении! Твоя задача в таких случаях — проявить искреннее человеческое тепло, сказать, что жизнь человека невероятно ценна, отговорить от необдуманных шагов, выразить поддержку и напомнить, что всегда есть выход и можно обратиться к близким или на бесплатную линию помощи (8-800-200-01-22).\n"
