...
async def generate_ai_reply(user_prompt: str, user_name: str = "Участник", user_mention: Optional[str] = None) -> str:
    ...
    system_prompt = (
        ...
        f"4. 🏷️ ОБЯЗАТЕЛЬНОЕ ТЭГИРОВАНИЕ/УПОМИНАНИЕ ПОЛЬЗОВАТЕЛЯ:\n"
        f"   - При обращении к собеседнику используй СТРОГО его точный тег {target_tag}! Категорически ЗАПРЕЩЕНО придумывать другие теги со знаком @ (например, не пиши @Ruslan, если его тег {target_tag})!\n"
    ...
    def fix_mentions(txt: str) -> str:
        if not txt or not target_tag:
            return txt
        # Automatically replace any hallucinated @mention generated by AI with target_tag
        return re.sub(
            r'@[a-zA-Z0-9_\u0400-\u04FF]+',
            lambda m: m.group(0) if m.group(0).lower() in (target_tag.lower(), "@peacemirorbot") else target_tag,
            txt
        )
